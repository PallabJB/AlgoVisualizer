export async function dfs(grid, start, end, refArray, speed = 10, setStats) {
  ...
  async function dfsVisit(x, y) {
    if (found || visited[`${x}-${y}`] || grid[y][x].iswall) return;
    visited[`${x}-${y}`] = true;
    nodesVisited++;
    const idx = y * width + x;
    ...
    for (const [dx, dy] of [[0,1],[1,0],[0,-1],[-1,0]]) {
      ...
      if (
        ...
        !visited[`${nx}-${ny}`] && !grid[ny][nx].iswall
      ) {
        prev[`${nx}-${ny}`] = { x, y };
        await dfsVisit(nx, ny);
      }
    }
  }

  await dfsVisit(start.x, start.y);
